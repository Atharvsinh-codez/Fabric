...
- the lease and retention values from `.env.example`

Vercel claims only the authenticated run attached to the current SSE request and keeps the bounded dispatch promise tied to that response lifecycle. Canvas-agent v2 permits one compact semantic planning turn with low reasoning, at most 4,096 output tokens, and a 60-second wall-clock deadline; the proposal route retains a 300-second Vercel envelope for dispatch, persisted SSE delivery, reconnects, and backward-compatible runs. Fabric rebuilds the selected and visible scene from the durable board, gives the provider opaque handles plus run-bound visual capabilities, and compiles the returned BoardPlan into deterministic native IDs, geometry, collision-free layout, and dependency-ordered operations. The model never owns tenant identifiers or absolute canvas coordinates. Model context is deterministically capped at 24,000 bytes inside the 40,000-byte authenticated request limit, prioritizing selected objects and then nearby visible context. These limits are reviewed code, not operator-adjustable `AI_MAX_*` environment values. The provider adapter sends only streamed OpenAI-compatible Chat Completions requests and never falls back to a buffered call. A semantic scene preview, a high-resolution selected-drawing crop, and exact selected/visible R2 images use short-lived HTTPS capabilities bound to the active run, board, selection, deadline, and exact asset hash. The media route revalidates every claim, derives purpose-separated signing material from `AUTH_SECRET`, and never exposes permanent R2 URLs. A request sends at most five visual inputs: scene plus crop plus three exact images, or scene plus four exact images. The browser cannot replace the provider endpoint, model, response schema, scene context, visual capabilities, compiler, or approval limits. Terminal event retention defaults to 14 days and terminal run retention to 30 days.
