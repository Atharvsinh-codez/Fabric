...
Vercel claims only the authenticated run attached to the current SSE request and keeps the bounded dispatch promise tied to that response lifecycle. The checked-in canvas-agent manifest permits one model turn within a 180-second wall-clock deadline, and the proposal route reserves a 300-second Vercel function envelope for dispatch plus persisted SSE delivery. These limits are reviewed code, not operator-adjustable `AI_MAX_*` environment values; use a Vercel plan that supports the committed function duration. The provider adapter sends only streamed OpenAI-compatible Chat Completions requests; it never falls back to a buffered or non-streaming model call. Selected drawings and images are exposed to the provider only through short-lived HTTPS capabilities bound to the active run, board, deadline, and exact asset hash; the media route revalidates those claims before streaming, derives its purpose-separated signing material from the existing `AUTH_SECRET`, and does not expose permanent R2 URLs. The browser cannot replace the provider endpoint, model, response schema, visual-context capabilities, or safety limits. Terminal event retention defaults to 14 days and terminal run retention to 30 days.
